...
const mariaPrices: Record<string, number> = {
  "pkg-1": 5500,
  "pkg-2": 15000,
  "pkg-3": 5999,
  "pkg-4": 10599,
...
function planAmount(projectId: string, rawPlanId: string, requestedAmount: unknown) {
  if (projectId === "mariapawar" || projectId === "proxima") {
    const canonical = canonicalPlanId(projectId, rawPlanId);
    const amount = mariaPrices[canonical];
    if (!amount) throw new Error("Invalid plan_id");
    return { canonical, amount };
  }

  const amount = Number(requestedAmount);
  if (!Number.isFinite(amount) || amount <= 0) throw new Error("Invalid amount");
  return { canonical: rawPlanId, amount: Math.round(amount) };
}
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === "/api/forms/submit") {
        ...
        if (!name || !email || (projectId === "mariapawar" && (!phone || !planId))) {
